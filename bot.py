...
отказавшихся подчиняться стирающий бесжалосно, даже не
...
подконтрольно всё воздушное пространство.''',
        'the_olympian': '''Превзошедший бессмертных богов... <u>Человек</u>...''',
}
...
Здравствуйте, <b>{input_message.from_user.first_name} {input_message.from_user.last_name}</b>!
...
def show_donates(input_message):
    ...
    the_olympian = types.KeyboardButton('Олимпиец')
    markup.add(
        centaur,
        griffin,
        chimera,
    ...
    def message_catch(input_message):
        match input_message.text:
            ...
            case 'Олимпиец':
                bot.send_message(input_message.chat.id, f"<b>{donates['the_olympian']}</b>", parse_mode='html')


@bot.message_handler(commands=['website'])
def new_markup(input_message):
    markup = types.InlineKeyboardMarkup()
    website_link = types.InlineKeyboardButton(text='САЙТ', url='https://www.youtube.com/watch?v=1GrOo3SccEY')
